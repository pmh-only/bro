...
When completed work has deployment or operational requirements, OpenCode can use the `operational_details` tool to send configuration steps, persistent paths, ports, and similar information as a separate Discord message.
...
```sh
docker run -it -p 8080:8080 -p 8081:8081 -p 8082:8082 -p 4096:4096 \
  -e DISCORD_TOKEN=... \
...
* The operational-details tool is installed globally in OpenCode. Its handoff files use `BRO_OPERATIONAL_DETAILS_DIR` (default `/home/opencode/data/operational-details`), which must be shared with the bot and stored persistently when OpenCode runs separately.
