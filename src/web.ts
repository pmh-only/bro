...
function page(threads: ProjectThread[], selectedProject: string | null): string {
  ...
  const activeJobs = jobs.filter((job) => job.state === "queued" || job.state === "running" || job.state === "cancelling").length;
