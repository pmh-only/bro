...
export class OpenCodeService {
  ...
  private routingPrompt(request: string, projectAliases: string[], routableJobs: RoutableJob[]): string {
    return [
      ...
      "- history: show or hide completed, failed, and cancelled jobs in the Web UI. Set historyVisible to true to show them or false to hide them. When the user identifies one history entry by job ID, also set jobId so only that entry is changed.",
